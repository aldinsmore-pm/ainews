...
Return only pure JSON in the specified format (no extra text, no markdown, no \`\`\`).
        ...
        try {
          if (source.includes('news.ycombinator.com')) {
            console.log('--- DEBUG: Scraping Hacker News (news.ycombinator.com) ---');
          }
          ...
          if (source.includes('news.ycombinator.com')) {
            console.log('--- DEBUG: Raw Firecrawl result for Hacker News:', JSON.stringify(scrapeResult, null, 2));
          }
          ...
          if (source.includes('news.ycombinator.com')) {
            if (todayStories.stories.length === 0) {
              console.log('--- DEBUG: No stories found from Hacker News ---');
            } else {
              console.log('--- DEBUG: Parsed stories from Hacker News:', todayStories.stories);
            }
          }
